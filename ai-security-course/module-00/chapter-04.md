...
**Module 00** · **Chapter 4** · **Complete** · **CTI evidence base**
...
Video companion: [Watch “Securing the LLM Request Path” on YouTube](https://www.youtube.com/watch?v=B0lSWuG2UUQ).
...
EchoLeak anchors this pattern in a disclosed production-system vulnerability. Aim Security researchers reported a zero-click indirect prompt-injection chain affecting Microsoft 365 Copilot; Microsoft assigned [CVE-2025-32711](https://msrc.microsoft.com/update-guide/vulnerability/CVE-2025-32711), remediated the issue, and stated that no customer action was required. MITRE ATLAS records the research as case study [`AML.CS0059`](https://github.com/mitre-atlas/atlas-data/blob/v2026.07/dist/v6/ATLAS-2026.07.yaml#L8104-L8145). The ATLAS case record says a malicious email could enter Copilot's retrieval context, influence the generated response, and use rendered Markdown image behavior to create an outbound exfiltration path.
...
*Layered controls for LLM-enabled systems — secure the build and release, context assembly, generation and parsing, tool execution, monitoring, and response as one evidence-producing system; the mapping panel uses MITRE ATLAS v2026.07. [Open infographic ↗](https://1200km.com/ai-security-course/assets/chapter-04/19-layered-controls.png)*
...
| ATLAS ID and title in 2026.07 | Chapter connection | Evidence required before mapping |
|---|---|---|
| [`AML.T0051.000`](https://github.com/mitre-atlas/atlas-data/blob/v2026.07/dist/v6/ATLAS-2026.07.yaml#L2609-L2701) — LLM Prompt Injection: Direct | Adversary supplies the instruction through the interactive input | Original request, identity, template, output, and protected boundary affected |
| [`AML.T0051.001`](https://github.com/mitre-atlas/atlas-data/blob/v2026.07/dist/v6/ATLAS-2026.07.yaml#L2609-L2701) — LLM Prompt Injection: Indirect | Separate data channel is ingested by the LLM | Retrieved object, source provenance, parser output, context inclusion, and generated result |
| [`AML.T0051.002`](https://github.com/mitre-atlas/atlas-data/blob/v2026.07/dist/v6/ATLAS-2026.07.yaml#L2609-L2701) — LLM Prompt Injection: Triggered | Later user action or event activates a planted instruction | Infiltration event, activation trigger, request trace, and resulting behavior |
| [`AML.T0070`](https://github.com/mitre-atlas/atlas-data/blob/v2026.07/dist/v6/ATLAS-2026.07.yaml#L3379-L3400) — RAG Poisoning | Manipulated content is indexed and later retrieved | Indexed object, index version, retrieval trace, relevance, and source authorization |
| [`AML.T0077`](https://github.com/mitre-atlas/atlas-data/blob/v2026.07/dist/v6/ATLAS-2026.07.yaml#L3546-L3571) — LLM Response Rendering | Rendered output creates an external request containing protected data | Raw model output, renderer behavior, outbound request, destination, and protected value linkage |
| [`AML.T0085.000`](https://github.com/mitre-atlas/atlas-data/blob/v2026.07/dist/v6/ATLAS-2026.07.yaml#L3868-L3945) — Data from AI Services: RAG Databases | AI service retrieves sensitive organizational information | Caller and tenant, source object, ACL decision, retrieved content, and output linkage |
| [`AML.T0086`](https://github.com/mitre-atlas/atlas-data/blob/v2026.07/dist/v6/ATLAS-2026.07.yaml#L3868-L3945) — Exfiltration via AI Agent Tool Invocation | A write-capable tool carries protected data to an adversary-controlled destination | Proposed arguments, policy decision, executing identity, API result, destination, and received data |

![MITRE ATLAS v2026.07 mapping table connecting direct, indirect, and triggered prompt injection, RAG poisoning, response rendering, data retrieval, and AI-agent tool exfiltration to required evidence](/ai-security-course/assets/chapter-04/20-atlas-mapping.png)
...
Complete the [Chapter 4 request-trace assessment in the Module 00 workbook](https://1200km.com/ai-security-course/module-00-workbook.html#chapter-04-assessment) and submit one concise evidence pack containing:
...
![LLM lifecycle roadmap with model acquisition, release, serving, monitoring, and retirement plus optional instruction-tuning, preference-optimization, adapter, and quantization branches](/ai-security-course/assets/chapter-04/24-what-comes-next.png)
...
2. Hugging Face Transformers v5.15.1, [Chat templates](https://huggingface.co/docs/transformers/v5.15.1/chat_templating), [Tokenizer summary](https://huggingface.co/docs/transformers/v5.15.1/tokenizer_summary), and [KV cache strategies](https://huggingface.co/docs/transformers/v5.15.1/kv_cache).
3. Hugging Face Transformers v5.15.1, [Generation strategies](https://huggingface.co/docs/transformers/v5.15.1/generation_strategies).
...
6. MITRE, [ATLAS](https://atlas.mitre.org/) and [ATLAS content release 2026.07 (Git tag `v2026.07`)](https://github.com/mitre-atlas/atlas-data/releases/tag/v2026.07), accessed 31 August 2026.
7. Greshake, K. et al., [Not what you've signed up for: Compromising Real-World LLM-Integrated Applications with Indirect Prompt Injection](https://arxiv.org/abs/2302.12173), 2023.
...
9. MITRE ATLAS, [EchoLeak: Zero-Click Prompt Injection Targeting M365 Copilot for Data Exfiltration (`AML.CS0059`)](https://github.com/mitre-atlas/atlas-data/blob/v2026.07/dist/v6/ATLAS-2026.07.yaml#L8104-L8145), ATLAS content release 2026.07.
...
11. Pavan Reddy and Aditya Sanjay Gujral, [EchoLeak: The First Real-World Zero-Click Prompt Injection Exploit in a Production LLM System](https://doi.org/10.1609/aaaiss.v7i1.36899), AAAI Symposium Series, 2025.
12. Su, J. et al., [RoFormer: Enhanced Transformer with Rotary Position Embedding](https://arxiv.org/abs/2104.09864), 2021.
13. Jain, S. and Wallace, B. C., [Attention is not Explanation](https://aclanthology.org/N19-1357/), NAACL-HLT, 2019.

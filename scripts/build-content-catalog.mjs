...
for (let offset = 0; offset < catalogueEntries.length; offset += concurrency) {
  ...
  for (const page of batch.filter(Boolean)) {
    ...
    if (!validation.indexable) {
      // A page whose own content says noindex is a stale sitemap entry, not
      // a broken build: sitemaps (this repo's remote-sitemaps.json sources
      // especially - see seo/remote-sitemaps.json) are generated by their
      // own separate pipelines and can lag behind a since-published noindex
      // change until their next deploy. Treating that as fatal here creates
      // a deadlock: this check blocks the deploy that would otherwise be
      // what refreshes the remote sitemap. Every other validation failure
      // (off-origin, not-html, redirect, canonical mismatch) still fails
      // loudly, since those indicate the sitemap references something
      // that's actually broken, not just newly de-indexed.
      if (validation.reason === 'noindex') {
        console.warn(`Skipping stale noindex sitemap entry: ${page.url}`);
        continue;
      }

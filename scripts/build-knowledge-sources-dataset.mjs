...
const vendor = new Set(['Check Point Research','SentinelOne Labs','Google Threat Intelligence','Cisco Talos Intelligence','Unit 42','CrowdStrike Global Threat Report','Verizon Data Breach Investigations Report','VirusTotal','IDA Free','Binary Ninja','Autopsy','Microsoft Sentinel Content Hub','Google SecOps Community Rules','AWS Security Best Practices','Microsoft Azure Security Documentation','Google Cloud Security Best Practices','Purple Knight','TryHackMe','LetsDefend','PentesterLab','Hack The Box Academy']);
const academic = new Set(['arXiv Cryptography and Security','USENIX Security Symposium']);
const sourceKindOverrides = new Map([
  ['MISP', 'open-source'],
  ['OpenCTI', 'open-source'],
  ['Open Source Vulnerabilities', 'open-source'],
  ['Zero Day Initiative', 'commercial-technical'],
  ['Trivy', 'open-source'],
  ['Kubescape', 'open-source'],
  ['Stratus Red Team', 'open-source'],
  ['MobSF', 'open-source'],
  ['Frida', 'open-source'],
  ['Semgrep', 'open-core'],
  ['Prowler', 'open-core'],
  ['Promptfoo', 'open-core'],
  ['BloodHound', 'open-core']
  ,['CodeQL', 'mixed-license-tool']
]);
...
const sources = rows.map(([name, url, category]) => {
  ...
  const kind = sourceKindOverrides.get(name) || (academic.has(name) ? 'academic' : vendor.has(name) ? 'commercial-technical' : url.includes('github.com') ? 'open-source' : 'authoritative-primary');
  const score = kind === 'authoritative-primary' ? 95 : kind === 'open-source' ? 90 : kind === 'academic' ? 90 : 82;
  return {
    ...
    quality: {
      ...
      rationale: kind === 'commercial-technical'
        ...
        : kind === 'academic'
          ...
          : kind === 'open-source'
            ? 'Official project source with direct operational or educational value.'
            : 'Canonical standards, government, project, or first-party documentation source.'
    },
    ...
    validation: { checked_on: checkedOn, method: 'automated HTTP check plus source-authority review', status: 'pending' }
  };
});
...
for (const source of unique.values()) {
  if (assessments[source.id]) {
    ...
    const transparency = source.source_kind === 'commercial-technical' ? 3.5 : ['open-source', 'academic', 'authoritative-primary'].includes(source.source_kind) ? 5 : 4;
    ...
    if (source.source_kind === 'open-source') {
    ...
    if (source.url.includes('github.com')) normalized.add('repositories');
    ...
    source.tags = [...normalized].filter(tag => controlledTags.has(tag)).slice(0, 10);
  }
}
...
fs.writeFileSync('data/knowledge-sources.json', `${JSON.stringify(output, null, 2)}\n`);
